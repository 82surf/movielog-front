<template>
  <div>
    <div class="modal fade" :id="`LikeModal${reviewData.id}`" tabindex="-1" data-bs-backdrop="static" aria-hidden="true">
      <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content" @keyup.esc="onCloseBtn">
          ...
          <div class="modal-body">
            ...
            <div class="collapse multi-collapse mb-3" :id="`like-modal-video-collapse-${reviewData.id}`">
          ...
          <div class="modal-footer">
            <button
              ...
              :id="`like-modal-collapse-btn-${reviewData.id}`"
              type="button"
              data-bs-toggle="collapse"
              :data-bs-target="`#like-modal-video-collapse-${reviewData.id}`"
              @click="onCollapseBtn">
              트레일러보기
            </button>
          </div>
...
<script>
export default {
  name: 'MovieListByLikesItemModal',
  data: function() {
    return {
      isPlay: false,
      inHtml: `<div></div>`,
  ...
  methods: {
    ...
    onCloseBtn: function() {
      if(this.videoKey !=='nothing'){
        if (this.isPlay){
          const collapseBtn = document.querySelector(`#like-modal-collapse-btn-${this.reviewData.id}`)
          collapseBtn.click()
        }
        this.inHtml = '<div>  </div>'
      } else{
